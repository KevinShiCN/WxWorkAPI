...
source: "https://developer.work.weixin.qq.com/document/path/95818"
...
- 品牌有自己主体（或isv主体）的公众号、小程序
- 品牌创建上下游，经销商新建企业并加入上下游
- 品牌共享应用给经销商
- 经销商管理员授权应用获取「企业客户」权限
...
- 品牌提前把公众号、小程序粉丝的 unionid （从微信开放平台获取）转换成 pending_id
- 经销商用企微添加微信客户时，用 external_userid 查询 pending_id
- unionid 和 external_userid 数据打通
...
| mass_call_ticket | 否 | 大批量调用凭据, 适用于数据初始化场景, 有获取及使用限制 |
