---
title: "接收消息和事件"
source: "https://developer.work.weixin.qq.com/document/path/94670"
...
微信客户发送的消息、接待人员在企业微信回复的消息、发送消息接口发送失败事件（如被用户拒收）、客户点击菜单消息的回复消息，可以通过该接口获取**最近3天内**具体的消息内容和事件。**不支持读取通过发送消息接口发送的消息**。
