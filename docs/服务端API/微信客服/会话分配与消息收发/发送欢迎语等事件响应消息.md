---
title: "发送欢迎语等事件响应消息"
source: "https://developer.work.weixin.qq.com/document/path/95122"
...
> 「进入会话事件」响应消息：
如果满足通过API下发欢迎语条件（条件为：**用户在过去48小时里未收过欢迎语，且未向客服发过消息**），则用户进入会话事件会额外返回一个`welcome_code`，开发者以此为凭据调用接口（填到该接口`code`参数），即可向客户发送客服欢迎语。
