...
> `func`固定为`sync_msg`，此时`func_req`为获取会话记录对应的输入协议
...
调用应用同步调用专区程序接口时，外层`notify_id`传入应用接收专区通知回调的`NotifyId`字段
...
- 企业产生新的会话事件后（下图1.1），企微专区后台会产生会话回调通知专区程序（下图1.2），回调中包含用于获取会话记录的token
- 详见：[产生会话回调通知](https://developer.work.weixin.qq.com/document/53357)
- 专区程序接收回调后，需要企业应用来拉取数据时，调用spec_notify_app通知企业应用（下图2.1），这里会给专区程序返回一份notify id，并向企业应用的回调地址发送一份包含**相同的notify id**的回调（下图2.2），专区程序需要将该notify id和待应用查询的数据作关联
- 详见：[专区通知应用](https://developer.work.weixin.qq.com/document/53408)
- 应用收到企微专区通道的回调后（下图2.2），请求专区程序（下图3.1），获取专区程序暂存的token
- 请求的ability id为get_callback_data
- 请求需要捎带notify id
- 请求为同步调用
- 应用拿到token后，可调用专区程序获取会话记录
