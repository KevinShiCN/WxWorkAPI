---
title: "自有模型分析"
source: "https://developer.work.weixin.qq.com/document/path/99985"
...
对于上传的大语言模型，需要在5秒内完成http请求的回包（回包的内容将会被忽略，但仍需要用sdk进行回包构造），之后通过调用上传异步任务结果接口来返回分析的结果。
...
| tag_group_list.group_id | 是 | 每个id代表一个客户标签组。 |
...
| debug_info.tagjson | 否 | 将用于替换{tagjson}占位符建议将类似正式调用时替换结果的json结构转义后作为字符串传入，以获得与正式调用时更一致的调试体验 |
